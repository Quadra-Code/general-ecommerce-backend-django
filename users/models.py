...
class CustomUser(AbstractUser):
    ...
    def to_dict(self):
        return {
            ...
            'main_phone_number': self.userphonenumbers_set.filter(is_main_number=True).first().phone_number.strip(),
            'main_address': self.useraddresses_set.filter(is_main_address=True).first().address.strip(),
            'email': self.email,
            'email_confirmed': self.email_confirmed,
            'date_joined': self.date_joined.strftime('%a %d %b %Y, %H:%M:%S %p'),
            'last_login': self.last_login.strftime('%a %d %b %Y, %H:%M:%S %p'),
    ...
    def __str__(self):
        ...
        phone_str = f"| Main Phone: {main_phone_number.phone_number}" if main_phone_number else ""
        ...
        return f"User Data => ID: {self.id} | Full Name: {full_name} {phone_str} {address_str}".strip()
...
class UserAddresses(BaseEntity):
    ...
    def __str__(self):
        return f"User Full Name => {self.user.first_name} {self.user.last_name} | Country => {self.country} | City => {self.city} | Address => {self.address}{f' | Postal Code => {self.postal_code}' if self.postal_code else ''}"
